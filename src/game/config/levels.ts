...
import { BoardGenerationOptions, createHexBoardBlueprint } from "./board";
import { HexCoord } from "../../core/topology/hexTopology";
import { GridBlueprint } from "../../core/grid/GridBlueprint";

interface LevelConfigFile {
  randomSeed?: number;
  levelCount?: number;
  wavesPerLevel?: number;
  startingDensity?: number;
  endingDensity?: number;
  tileProportions?: Partial<Record<'normal' | 'hole' | 'clearable' | 'water' | 'wall', number>>;
  clearCost?: number;
}
...
export interface StaticLevelSettings {
  clearCost: number;
  tileWeights: BoardGenerationOptions['tileWeights'];
  levels: LevelDefinition[];
}

const FALLBACK_CONFIG: Required<LevelConfigFile> = {
  randomSeed: 1337,
  levelCount: 6,
  wavesPerLevel: 30,
...
const rawConfig = (levelSettingsFile as LevelConfigFile) ?? {};
const mergedConfig: Required<LevelConfigFile> = {
...
export const staticLevelSettings: StaticLevelSettings = {
...
export const createLevelBlueprint = (
  radius: number,
...
function buildLevelDefinitions(config: Required<LevelConfigFile>): LevelDefinition[] {
...
function normalizeProportions(
  proportions: Partial<Record<'normal' | 'hole' | 'clearable' | 'water' | 'wall', number>>
): Required<Record<'normal' | 'hole' | 'clearable' | 'water' | 'wall', number>> {
...
function scrambleSeed(seed: number, index: number): number {
  const base = seed >>> 0;
