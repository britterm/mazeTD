...
export class GameEngine<Coord> {
  ...
  clearTerrain(coord: Coord, cost: number): { success: boolean; reason?: string; towerId?: string } {
  ...
  private placeTowerWithoutCost(coord: Coord, towerType: string): string | null {
    const def = this.getTowerDefinition(towerType);
    if (!def) {
      return null;
    }
